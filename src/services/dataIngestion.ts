...
const PARTY_COLORS: Record<string, string> = {
  ...
  'ГЕРБ - СДС': '#0066CC',
  'ПП-ДБ': '#FFD700',
  'Продължаваме промяната – Демократична България': '#FFD700',
  'ДПС': '#003366',
  'ДПС – Ново начало': '#003366',
  'Движение за права и свободи – Ново начало': '#003366',
  'Възраждане': '#8B0000',
  'ВЪЗРАЖДАНЕ': '#8B0000',
  'БСП': '#CC0000',
  'БСП – ОБЕДИНЕНА ЛЕВИЦА': '#CC0000',
  'ИТН': '#00CED1',
  'Има Такъв Народ': '#00CED1',
  'ПП МЕЧ': '#9C27B0',
  'Алианс за права и свободи': '#FF9800',
  'ВЕЛИЧИЕ': '#4CAF50',
  'Нечленуващи в ПГ': '#666666',
}
  ...
  async syncParties(deputies: ParliamentMPRaw[]): Promise<Map<string, number>> {
    ...
    deputies.forEach(deputy => {
      ...
      if (!uniqueParties.has(partyName)) {
        // Извличаме реалното име на партията (махаме "Парламентарна група")
        let cleanName = partyName
          .replace(/^Парламентарна група\s+/i, '')
          .replace(/^на\s+/i, '')
          .replace(/^["«»]/g, '')
          .replace(/["«»]$/g, '')
          .trim()
        
        // Ако името започва с кавички, вземи текста между кавички
        const quotedMatch = partyName.match(/["«»]([^"«»]+)["«»]/)
        if (quotedMatch) {
          cleanName = quotedMatch[1].trim()
        }
        
        uniqueParties.set(partyName, {
          name: cleanName,
          shortName: cleanName.split(/\s+[-–—]\s+/)[0].trim() || cleanName.substring(0, 10),
        })
      }
    })
