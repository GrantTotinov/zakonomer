...
export class DataIngestionService {
  ...
  async fetchDeputiesFromAPI(): Promise<ParliamentMPRaw[]> {
    try {
      ...
      if (result.colListMP) {
        console.log(`Successfully fetched ${result.colListMP.length} deputies`)
        return result.colListMP
      }
      
      throw new Error('Invalid API response format - missing colListMP')
    } catch (error) {
      console.error('Error fetching deputies from API:', error)
      
      // Ако има CORS грешка, пробвай през Supabase Edge Function
      if (error instanceof TypeError && error.message.includes('CORS')) {
        console.log('CORS error detected, trying via Edge Function...')
  ...
  private async fetchDeputiesViaProxy(): Promise<ParliamentMPRaw[]> {
    const response = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/parliament-proxy?endpoint=coll-list-ns/bg`,
      {
        headers: {
